...
public sealed class StackService : IStackService
{
    ...
    private async Task<List<ContainerStatusDto>> GetContainersAsync(string stackId, CancellationToken cancellationToken)
    {
        try
        {
            var containers = await _dockerService.ListContainersAsync(DockerComposeOverrideGenerator.GetComposeProjectName(stackId), cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        await File.WriteAllTextAsync(
            overridePath,
            GenerateRuntimeDockerComposeOverride(stack, composeProjectName, repoPath),
            cancellationToken);
    }

    private string GenerateRuntimeDockerComposeOverride(ManagedStackEntity stack, string composeProjectName, string repoPath)
    {
        var customEnvironment = Deserialize<Dictionary<string, string>>(stack.CustomEnvVarsJson)
            ?? new Dictionary<string, string>();

        // Translate modules path for Docker-in-Docker
        var modulesPath = TranslateToHostPath(Path.Combine(repoPath, "modules"));

        return DockerComposeOverrideGenerator.Generate(stack.Id, customEnvironment, modulesPath);
    }
